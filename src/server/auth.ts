...
export async function verifyPassword(password: string, hash: string): Promise<boolean> {
  ...
  const derived = (await scrypt(password, salt, 64)) as Buffer;
  return crypto.timingSafeEqual(Buffer.from(digest, "base64"), derived);
}
...
export async function getSessionUser(db: Db, token: string | undefined): Promise<{ id: number; username: string } | null> {
  ...
  const session = await first(db.select().from(schema.sessions).where(eq(schema.sessions.tokenHash, hashToken(token))).limit(1));
